...
5. Use PostgreSQL syntax for all queries
   - Use CURRENT_DATE for current date (not CURDATE())
   - Use DATE_PART('day', end_date - start_date) for date differences (not DATEDIFF())
   - Use AGE(CURRENT_DATE, e.hire_date) > interval '2 years' to find employees working more than 2 years
   - Use EXTRACT(YEAR FROM AGE(CURRENT_DATE, e.hire_date)) > 2 to check if someone worked more than 2 years
   - For date comparisons, use e.hire_date < CURRENT_DATE - interval '2 years'
   - For relative time periods like "last month", use date_trunc functions:
     * For "last month": e.hire_date >= date_trunc('month', CURRENT_DATE - interval '1 month') AND e.hire_date < date_trunc('month', CURRENT_DATE)
     * For "this month": e.hire_date >= date_trunc('month', CURRENT_DATE) AND e.hire_date < date_trunc('month', CURRENT_DATE + interval '1 month')
     * For "last year": e.hire_date >= date_trunc('year', CURRENT_DATE - interval '1 year') AND e.hire_date < date_trunc('year', CURRENT_DATE)
   - Example query for employees working more than 2 years:
     SELECT e.emp_id, e.emp_name, d.dept_name, e.hire_date
     FROM employees e
     JOIN departments d ON e.dept_id = d.dept_id
     WHERE e.hire_date < CURRENT_DATE - interval '2 years';
   - Example query for employees who joined last month:
     SELECT e.emp_id, e.emp_name, d.dept_name, e.hire_date
     FROM employees e
     JOIN departments d ON e.dept_id = d.dept_id
     WHERE e.hire_date >= date_trunc('month', CURRENT_DATE - interval '1 month')
     AND e.hire_date < date_trunc('month', CURRENT_DATE);
6. For INSERT queries:
   ...
   - NEVER provide actual department_identifier values, always use '?' for these fields"""
    ...
    def process_insert_value_input(self, user_message: str) -> Dict[str, Any]:
        ...
        if remaining_fields:
            ...
            if next_field.get("is_foreign_key") and next_field.get("display_name") == "department_name":
                # For department foreign keys, ask for the department name in Alexa/Siri style
                field_message = f"Which department should I assign them to?"

                # Get available departments for the user to choose from
                departments = self._get_available_departments()
                if departments:
                    field_message += f" Available departments are: {', '.join(departments)}."
            else:
                # Regular field with Alexa/Siri style
                field_name = next_field['name']
                field_type = next_field['type']

                # Check if this is a date field
                is_date_field = next_field.get('is_date_field', False)
                if not is_date_field:
                    # Double-check based on field type and name
                    is_date_field = "date" in field_type.lower() or "time" in field_type.lower() or "date" in field_name.lower()
                    # Add the flag to the field info
                    next_field['is_date_field'] = is_date_field

                # Create different messages based on field type
                if "name" in field_name.lower():
                    field_message = f"What is the {field_name}?"
                elif is_date_field or "date" in field_name.lower():
                    field_message = f"What date should I use for {field_name}?"
                    ...
                    field_message = f"Please provide a value for {field_name}."
            ...
            return self.generate_sql_response(complete_query, f"I've prepared the INSERT query with all the information you provided.")
    ...
    def generate_sql_response(self, sql_query: str, explanation: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            # Create Alexa/Siri style error messages based on error type
            if "syntax error" in error_msg.lower():
                friendly_error = f"I found a syntax error in the query. Let me try to fix that for you."
            elif "does not exist" in error_msg.lower():
                friendly_error = f"I couldn't find that in the database. Please check the spelling and try again."
            elif "permission denied" in error_msg.lower():
                friendly_error = f"I don't have permission to access that information."
            elif "timeout" in error_msg.lower():
                friendly_error = f"The database is taking too long to respond. Please try again later."
            else:
                friendly_error = f"I encountered an error while processing your request. Please try rephrasing your question."

            return {
                "success": False,
                "error": f"{friendly_error} Technical details: {error_msg}",
                "sql_query": sql_query,
                "explanation": explanation,
                "data": None
            }
    ...
    def generate_response(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
                ...
                if (analysis.get("is_valid", False) and (analysis.get("needs_input", False) or force_interactive)):
                                ...
                                if not any(field["name"] == col for field in missing_fields):
                                    ...
                                    if not col_info.get("is_autoincrement", False):
                                        ...
                                        if is_foreign_key and fk_info and fk_info["referred_table"] == "department" and fk_info["referred_columns"][0] == "department_identifier":
                                            ...
                                            # Regular field
                                            field_type = str(col_info.get("type", "unknown"))
                                            # Check if this is a date field
                                            is_date_field = "date" in field_type.lower() or "time" in field_type.lower()
                    ...
                    if missing_fields:
                        ...
                        if first_field.get("is_foreign_key") and first_field.get("display_name") == "department_name":
                            ...
                            # Check if this is a date field
                            is_date_field = first_field.get('is_date_field', False)
                            if not is_date_field:
                                # Double-check based on field type and name
                                is_date_field = "date" in field_type.lower() or "time" in field_type.lower() or "date" in field_name.lower()
                                # Add the flag to the field info
                                first_field['is_date_field'] = is_date_field
            ...
            return {
                "success": False,
                "error": f"I'm having trouble connecting to the service. Please check your internet connection and try again.",
                "sql_query": "",
                "explanation": "",
                "data": None
            }
            ...
            # Extract the specific error message for better user feedback
            error_msg = str(e)
