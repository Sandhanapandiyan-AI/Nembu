...
class DatabaseService:
            ...
            with self.engine.connect() as connection:
                ...
                try:
                    ...
                    # For SELECT queries, fetch the results
                        ...
                        # Convert rows to list of dicts for easier handling with special formatting
                        results = []
                        for row in rows:
                            row_dict = {}
                            for i, col in enumerate(columns):
                                ...
                                # Format years_worked column if it exists and contains large numbers (seconds)
                                if col == 'years_worked' and isinstance(value, (int, float)) and value > 31536000:  # More than 1 year in seconds
                                    # Convert seconds to years (approximate)
                                    years = round(value / 31536000, 1)  # 31536000 seconds in a year
                                    row_dict[col] = f"{years} years"
                                # Format salary values with Indian Rupee symbol
                                elif 'salary' in col.lower() and isinstance(value, (int, float)):
    ...
    def get_results_as_json(self, query_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Handle data modification queries (UPDATE, INSERT, DELETE)
        if "query_type" in query_results and query_results["query_type"] in ["UPDATE", "INSERT", "DELETE"]:
            # Professional Alexa/Siri style messages for modification queries
            if query_results["query_type"] == "INSERT":
                message = f"I've added {query_results['row_count']} row to the database."
            elif query_results["query_type"] == "UPDATE":
                message = f"I've updated {query_results['row_count']} rows in the database."
            elif query_results["query_type"] == "DELETE":
                message = f"I've deleted {query_results['row_count']} rows from the database."
            else:
        ...
        if query_results["row_count"] == 0:
            return {
                "success": True,
                "query_type": "SELECT",
                "message": "I couldn't find any matching records. Would you like to try a different search?",
                "data": {
                    "columns": [],
                    "rows": []
                }
        ...
        # Handle SELECT queries with results
        count = query_results['row_count']
        if count == 1:
            message = "I found 1 result for your query."
        else:
            message = f"I found {count} results for your query."
